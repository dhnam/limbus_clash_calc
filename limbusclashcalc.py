...
main_title_trans: SentencePair = {
    'kr': '림버스 합 승률 계산기',
    'en': 'Limbus Clash Winrate Calculator',
}

detail_title_trans: SentencePair = {
    'kr': '상세정보',
...
window = sg.Window(main_title_trans[language], layout, finalize=True)
...
while True:
    ...
    if event == "calc" or event == "detail":
        ...
        if event == "detail":
            if window2 is not None:
                continue
            window2 = sg.Window(detail_title_trans[language], detail_layout(language), finalize=True)
    if event.split("::")[-1] == "language":
        ...
        window = sg.Window(main_title_trans[language], main_layout(language), location=loc, finalize=True)
        del values[0]
        window.fill(values)
